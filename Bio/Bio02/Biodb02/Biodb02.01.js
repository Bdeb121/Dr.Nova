let quiz = {
    chapter: 'Biological Classification',
    topics: [
        'Monera',
        'Protista',
        'Fungi',
        'Plantae',
        'Animalia',
        'Viruses',
        'Viroids',
        'Lichens',
        'Prions',
    ],
    title: 'Biological Classification Ex1',
    colors: ['#096c86', '#096c8636', '#dbf6fb'],
    questions: [
        {
            question:
                "In Whittaker's system of classification, prokaryotes are placed in the kingdom",
            options: [
                'Protista ',
                'Monera',
                'Plantae ',
                'Animalia',
            ],
            answer: 2,
        },
        {
            question:
                'In the five kingdom system of classification, which single kingdom out of the following',
            statement: [
                'can include blue-green algae',
                'nitrogen fixing bacteria',
                'methanogenic archaebacteria',
            ],
            options: [
                'Monera ',
                'Fungi',
                'Plantae ',
                'Protista',
            ],
            answer: 1,
        },
        {
            question:
                'Which of the following kingdom does not have nuclear membrane?',
            options: [
                'Protista ',
                'Fungi',
                'Monera ',
                'Plantae',
            ],
            answer: 3,
        },
        {
            question:
                'What type of mode of nutrition is found in the kingdom Animalia?',
            options: [
                'Autotrophic and heterotrophic',
                'Chemosynthetic and photosynthetic',
                'Saprophytic and parasitic',
                'Holozoic and saprophytic',
            ],
            answer: 4,
        },
        {
            question:
                'The separation of living beings into five kingdoms is based on',
            options: [
                'Complexity of cell structure',
                "Complexity of organism's body",
                'Mode of obtaining nutrition',
                'All of the above',
            ],
            answer: 4,
        },
        {
            question: 'The chief component of bacterial cell wall is',
            options: [
                'Cellulose and chitin',
                'Cellulose and pectin',
                'Amino acids and polysaccharides',
                'Cellulose and carbohydrates',
            ],
            answer: 3,
        },
        {
            question: 'Bacteria whose cell has only a curve/comma is',
            options: [
                'Vibrio ',
                'Cocci',
                'Spirilli ',
                'Bacilli',
            ],
            answer: 1,
        },
        {
            question:
                'he main difference between Gram positive and Gram negative bacteria lies in the composition of',
            options: [
                'Cilia ',
                'Cell wall',
                'Nucleolus ',
                'Cytoplasm',
            ],
            answer: 2,
        },
        {
            question: 'Helically coiled shaped bacteria are called',
            options: [
                'Spirilla ',
                'Coed',
                'Bacilli ',
                'Vibrio',
            ],
            answer: 1,
        },
        {
            question: 'Cell wall is absent in',
            options: [
                'bacteria ',
                'fungi',
                'plants ',
                'animals',
            ],
            answer: 4,
        },
        {
            question:
                'Which one of the following organisms may respire in the absence of oxygen?',
            options: [
                'Azotobacter ',
                'Clostridium',
                'Rhizobium ',
                'Lactobacillus',
            ],
            answer: 2,
        },
        {
            question:
                'An organism having cytoplasm i.e. DNA and RNA but no cell wall is',
            options: [
                'Cyanobacterium ',
                'Mycoplasma',
                'Bacterium ',
                'Virus',
            ],
            answer: 2,
        },
        {
            question: 'Kingdom monera comprises the',
            options: [
                'Plants of economic importance',
                'All the plants studied in botany',
                'Prokaryotic organisms',
                'Plants of Thallophyta group',
            ],
            answer: 3,
        },
        {
            question: 'The cell wall of green plants is made up of',
            options: [
                'Pectin ',
                'Suberin',
                'Cellulose ',
                'Chitin',
            ],
            answer: 3,
        },
        {
            question: 'Which of the following is not a blue-green algae?',
            options: [
                'Nostoc ',
                'Anabaena',
                'Lichen ',
                'Aulosiras',
            ],
            answer: 3,
        },
        {
            question:
                'During rainy seasons, the ground becomes slippery due to dense growth of',
            options: [
                'Lichens ',
                'Bacteria',
                'Green algae ',
                'Cyanobacteria',
            ],
            answer: 4,
        },
        {
            question: 'Paramecium is a',
            options: [
                'Protozoan ',
                'Bacterium',
                'Virus ',
                'Annelid',
            ],
            answer: 1,
        },
        {
            question: 'Protists are',
            options: [
                'single-celled eukaryotes ',
                'multicellular eukaryotes',
                'single-celled prokaryotes ',
                'single-celled akaryote',
            ],
            answer: 1,
        },
        {
            question: 'Total parasites belong to protozoan group',
            options: [
                'Sporozoa ',
                'Ciliata',
                'Sarcodina ',
                'Zooflagellata',
            ],
            answer: 1,
        },
        {
            question: 'The cilia in Paramecium are',
            options: [
                'All equal ',
                'All unequal',
                'Longer at posterior end ',
                'Longer at anterior end',
            ],
            answer: 3,
        },
        {
            question: 'Plasmodium, the parasite, belongs to class',
            options: [
                'Sarocodina ',
                'Ciliata',
                'Sporozoa ',
                'Dinophyceae',
            ],
            answer: 3,
        },
        {
            question:
                'Which of the following organisms were never included in protista?',
            options: [
                'Bacteria ',
                'Red algae',
                'Slime moulds ',
                'Mosses',
            ],
            answer: 4,
        },
        {
            question:
                'Which of the following does not contain chlorophyll?',
            options: [
                'Fungi ',
                'Algae',
                'Bryophyta ',
                'Pteridophyta',
            ],
            answer: 1,
        },
        {
            question:
                'Which of the following statements are true about bacteria?',
            options: [
                'They are the sole members of the kingdom Monera.',
                'They live in extreme habitats such as hot springs, deserts, snow and deep oceans',
                'They show the most extensive metabolic diversity',
                'All of these',
            ],
            answer: 4,
        },
        {
            question: 'The cell wall of fungi is made up of',
            options: [
                'Chitin ',
                'Cellulose',
                'Pectin ',
                'Suberin',
            ],
            answer: 1,
        },
        {
            question:
                'The disease of potato responsible for famous famine of Europe was caused by or late blight of potato is caused by',
            options: [
                'Colletotrichum falcatum',
                'Phytophthora infestans',
                'Potato mosaic virus',
                'Alternaria solani',
            ],
            answer: 2,
        },
        {
            question: 'Ergot is caused by',
            options: [
                'Claviceps ',
                'Penicillium',
                'Aspergillus ',
                'Rhizobium',
            ],
            answer: 1,
        },
        {
            question:
                'When fungi feed on dead organic matter, they are known as',
            options: [
                'Dimorphic ',
                'Parasites',
                'Saprophytes ',
                'None of these',
            ],
            answer: 3,
        },
        {
            question:
                'Which of the following divisions of fungi includes Club fungi?',
            options: [
                'Zygomycota ',
                'Ascomycota',
                'Deuteromycota ',
                'Basidiomycota',
            ],
            answer: 4,
        },
        {
            question: 'Which of the following fungi are edible?',
            options: [
                'Agaricus campestris ',
                'Morchella esculenta',
                'Podaxon prodaxis ',
                'All of these',
            ],
            answer: 4,
        },
        {
            question: 'Gametangial copulation (conjugation) is common in',
            options: [
                'Zygomycetes ',
                'Ascomycetes',
                'Phycomycetes ',
                'Deuteromycetes',
            ],
            answer: 1,
        },
        {
            question: 'Dikaryon formation is characteristic of',
            options: [
                'Ascomycetes and basidiomycetes',
                'Phycomycetes and basidiomycetes',
                'Ascomycetes and phycomycetes',
                'Phycomycetes and zygomycetes',
            ],
            answer: 1,
        },
        {
            question: 'Plasmogamy is fusion of',
            options: [
                'Two haploid cells including their nuclei',
                'Two haploid cells without nuclear fusion',
                'Sperm and egg',
                'Sperm and two polar nuclei',
            ],
            answer: 2,
        },
        {
            question: 'Clamp connection is found in',
            options: [
                'Basidiomycetes ',
                'Ascomycetes',
                'Saccharomycetes ',
                'Haplomycetes',
            ],
            answer: 1,
        },
        {
            question: 'Difference between virus and viroid is',
            options: [
                'absence of protein coat in viroid but present in virus',
                'presence of low molecular weight RNA in virus but absent in viroid',
                'both and (b)',
                'None of these',
            ],
            answer: 1,
        },
        {
            question: 'Common bread mould is',
            options: [
                'Yeast ',
                'Rhizopus',
                'Bacteria ',
                'Virus',
            ],
            answer: 2,
        },
        {
            question:
                'Branched, aseptate, coenocytic mycelium is present in',
            options: [
                'Aspergillus ',
                'Albugo',
                'Penicillium ',
                'Erysiphae',
            ],
            answer: 2,
        },
        {
            question:
                'In manufacture of bread, it becomes porous due to release of CO<sub>2</sub> by the action of',
            options: [
                'Virus ',
                'Yeast',
                'Bacteria ',
                'Protozoans',
            ],
            answer: 2,
        },
        {
            question: 'Members of phycomycetes are found in',
            options: [
                'aquatic habitats ',
                'on decaying wood',
                'moist and damp places ',
                'all of these',
            ],
            answer: 4,
        },
        {
            question:
                "'Mycorrhizae' are useful for plants mainly due to their following attribut",
            options: [
                'Fixing atmospheric nitrogen',
                'Enhanced absorption of nutrients from soil',
                'Killing insects and pathogens',
                'Providing resistance against abiotic stresses',
            ],
            answer: 2,
        },
        {
            question: 'Red rot of sugarcane is caused by',
            options: [
                'Puccinia ',
                'Albugo',
                'Ustilago ',
                'Colletotrichum',
            ],
            answer: 4,
        },
        {
            question: 'Fungi are always',
            options: [
                'Autotrophs ',
                'Heterotrophs',
                'Saprophytes ',
                'Parasites',
            ],
            answer: 2,
        },
        {
            question:
                'Which of the following option is true for methanogens?',
            statement: [
                'They are eukaryotes.',
                'They live in marshy areas.',
                'They are also present in the guts of several ruminant animals such as cows and buffaloes.',
                'They are responsible for the production of methane (biogas)',
            ],
            options: [
                '1 and 2 ',
                '2 and 3',
                '1, 2, 3 and 4 ',
                'None of these',
            ],
            answer: 3,
        },
        {
            question: 'Pseudomycelium is characteristic feature of',
            options: [
                'Mushroom ',
                'Mucor',
                'Bread mould ',
                'Yeast',
            ],
            answer: 4,
        },
        {
            question: 'Common form of food stored in a fungal cell is',
            options: [
                'Glycogen ',
                'Starch',
                'Glucose ',
                'Sucrose',
            ],
            answer: 1,
        },
        {
            question:
                'Which of the following pigment present in cyanobacteria?',
            options: [
                'Chlorophyll ‘a’ ',
                'Chlorophyll ‘b’',
                'Chlorphyll ‘c’ ',
                'Chlorophyll ‘d’',
            ],
            answer: 1,
        },
        {
            question: 'Rhizopus belongs to the class',
            options: [
                'Ascomycetes ',
                'Phycomycetes',
                'Basidiomycetes ',
                'Deuteromycetes',
            ],
            answer: 2,
        },
        {
            question:
                'Which one of the following is smallest living cell and live without oxygen?',
            options: [
                'Mycoplasma ',
                'Mycorrhiza',
                'Euglena ',
                'Trypanosoma',
            ],
            answer: 1,
        },
        {
            question:
                'The Tobacco mosaic virus was crystallized for first time by',
            options: [
                'W. M. Stanley ',
                'E. C. Stackman',
                'A. K. Smith ',
                'Ivanowski',
            ],
            answer: 1,
        },
        {
            question: 'The genetic material in viruses is',
            options: [
                'Only RNA',
                'Only DNA',
                'RNA and DNA both',
                'RNA or DNA i.e. one nucleic acid in a virus',
            ],
            answer: 4,
        },
        {
            question:
                'Which of the following pair comes under the group chrysophytes?',
            options: [
                'Diatoms and Euglena',
                'Euglena and Trypanosoma',
                'Diatoms and desmids',
                'Gonyaulax and desmids',
            ],
            answer: 2,
        },
        {
            question: 'Which one is absent in viruses ?',
            options: [
                'Replication ',
                'Protein synthesis',
                'Energy liberation ',
                'Mutation',
            ],
            answer: 3,
        },
        {
            question: 'Protists obtain food as',
            options: [
                'photosynthesisers, symbionts and holotrophs',
                'photosynthesisers',
                'chemosynthesisers',
                'holotrophs',
            ],
            answer: 1,
        },
        {
            question:
                'The part of the virus which gives to it the hereditary feature, is',
            options: [
                'Capsid ',
                'Capsomere',
                'Nucleic acid ',
                'Nucleotide',
            ],
            answer: 4,
        },
        {
            question: 'A bacteriophage is',
            options: [
                'A virus attacking a bacterium',
                'A bacterium attacking a virus',
                'A stage in the life-cycle of bacterium',
                'A virus attacking another virus',
            ],
            answer: 1,
        },
        {
            question:
                'A few organisms are known to grow and multiply at temperatures of 100-105ºC. They belong to',
            options: [
                'marine archaebacteria',
                'thermophilic sulphur bacteria',
                'hot-spring blue-green algae (cyanobacteria)',
                'thermophilic, subaerial fungi',
            ],
            answer: 1,
        },
        {
            question:
                'The most abundant prokaryotes helpful to humans in making curd from milk and in production of antibiotics are the ones categorised as',
            options: [
                'Cyanobacteria',
                'Archaebacteria',
                'Chemosynthetic autotrophs',
                'Heterotrophic bacteria',
            ],
            answer: 4,
        },
        {
            question: 'The cyanobacteria are also referred to as',
            options: [
                'protists ',
                'golden algae',
                'slime moulds ',
                'blue green algae',
            ],
            answer: 4,
        },
        {
            question:
                'Lichens are important in studies on atmospheric pollution because they',
            options: [
                'can also grow in greatly polluted atmosphere',
                'can readily multiply in polluted atmosphere',
                'efficiently purify the atmosphere',
                'are very sensitive to pollutants',
            ],
            answer: 4,
        },
        {
            question:
                'The symbiotic association of fungi and algae is called',
            options: [
                'Lichen ',
                'Mycorrhiza',
                'Rhizome ',
                'Endomycorrhiza',
            ],
            answer: 1,
        },
        {
            question:
                'Which one of the following is not true about lichens?',
            options: [
                'Their body is composed of both algae and fungal cells.',
                'These grow very fast at the rate of about 2cm per year.',
                'Some form food for reindeer in arctic regions.',
                'Some species can be used as pollution indicators.',
            ],
            answer: 2,
        },
        {
            question:
                'There exists a close association between the alga and the fungus within a lichen. The fungus',
            options: [
                'provides protection, anchorage and absorption for the alga',
                'provides food for the alga',
                'fixes the atmospheric nitrogen for the alga',
                'releases oxygen for the alga',
            ],
            answer: 1,
        },
        {
            question:
                'The thalloid body of a slime mould (Myxomycetes) is known as',
            options: [
                'plasmodium ',
                'fruiting body',
                'mycelium ',
                'protonema',
            ],
            answer: 1,
        },
        {
            question:
                'Which pair of the following belongs to Basidiomycetes?',
            options: [
                'Puffballs and Claviceps',
                'Peziza and stink borns',
                'Morchella and mushrooms',
                'Birds nest fungi and puffballs.',
            ],
            answer: 4,
        },
        {
            question: 'Which one of the following is a slime mould?',
            options: [
                'Physarum ',
                'Thiobacillus',
                'Anabaena ',
                'Rhizopus',
            ],
            answer: 1,
        },
        {
            question:
                'Thermococcus, Methanococcus and Methanobacterium exemplify',
            options: [
                'Archaebacteria that contain protein homologous to eukaryotic core histones',
                'Archaebacteria that lack any histones resembling those found in eukaryotes but whose DNA is negatively supercoiled',
                'Bacteria whose DNA is relaxed or positively supercoiled but which have a cytoskeleton as well as mitochondria',
                'Bacteria that contain a cytoskeleton and ribosomes',
            ],
            answer: 1,
        },
        {
            question: 'Mannitol is the stored food in',
            options: [
                'Porphyra ',
                'Fucus',
                'Gracillaria ',
                'Chara',
            ],
            answer: 2,
        },
        {
            question: 'Single-celled eukaryotes are included in',
            options: [
                'Protista ',
                'Fungi',
                'Archaea ',
                'Monera',
            ],
            answer: 1,
        },
        ,
        {
            question: 'Ringworm in humans is caused by',
            options: [
                'Bacteria ',
                'Fungi',
                'Nematodes ',
                'Viruses',
            ],
            answer: 2,
        },
        {
            question:
                'Which one of the following organisms is not an example of eukaryotic cells?',
            options: [
                'Paramecium caudatum ',
                'Escherichia coli',
                'Euglena viridis ',
                'Amoeba proteus',
            ],
            answer: 2,
        },
        {
            question:
                'In eubacteria, a cellular component that resembles eukaryotic cells is',
            options: [
                'plasma membrane ',
                'nucleus',
                'ribosomes ',
                'cell wall',
            ],
            answer: 1,
        },
        {
            question: 'Organisms called methanogens are most abundant in a',
            options: [
                'sulphur rock ',
                'cattle yard',
                'polluted stream ',
                'hot spring',
            ],
            answer: 2,
        },
        {
            question:
                'The highest number of species in the world is represented by',
            options: ['Fungi ', 'Mosses', 'Algae ', 'Lichens'],
            answer: 1,
        },
        ,
        {
            question:
                'In the five-kingdom classification, Chlamydomonas and Chlorella have been included in',
            options: [
                'protista ',
                'algae',
                'plantae ',
                'monera',
            ],
            answer: 2,
        },
        {
            question:
                'Which one of the following pathogens causes canker disease?',
            options: [
                'Meloidogyne incognita',
                'Anguina tritici',
                'Xanthomonas citri',
                'Pseudomonas rubilineans',
            ],
            answer: 3,
        },
    ],
};
